...
methods = {plot_name[k]:v for k,v in globals().items() if k.startswith("norm")}
markers = cycle('h*o>Dxsp8')
linestyles = cycle(['-', ':', '--', '-.'])

if __name__ == "__main__":
    timings = {k: [] for k in methods}
    for exponent in range(12, 40):
        N = int(1.5 ** exponent)
    ...
    timings = list(timings.items())
    timings.sort(key = lambda k_v : np.mean([i[1] for i in k_v[1]]), reverse=True)
    for name, data in timings:
        d = np.asarray(data)
        py.plot(d[:, 0], d[:, 1], label=name, marker=next(markers),
                linestyle=next(linestyles), linewidth=4)
